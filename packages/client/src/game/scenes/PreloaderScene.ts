...
export class PreloaderScene extends Phaser.Scene {
  ...
  preload() {
    ...
    // Maps for every venue THAT HAS ONE (generated by scripts/world-bake.mjs).
    //
    // F-2: this used to walk `venueRegistry.all()`. D-79 published a venue per
    // parcel, and world-bake.mjs writes tilemaps for authored venues and
    // archetype instances only — plot venues are appended after that loop, on
    // purpose, because a vacant parcel has no interior to bake. Asking for all
    // 41 therefore asked for 23 files that were never written: 23 guaranteed
    // 404s on every single boot. Not cosmetic — Phaser's loader counts a
    // failed file toward `progress`, so the bar and the "Loading N%" text were
    // pacing against 23 requests that could only ever fail, on a phone over a
    // slow link as much as on localhost.
    for (const v of venueRegistry.withTilemap()) {
      this.load.tilemapTiledJSON(v.id, `assets/tilemaps/${v.id}.tmj`);
    }
    // The atlas list still comes from ALL venues: a parcel declares
    // `groundAtlas: 'district_ground'` and is drawn with it, so dropping
    // parcels here would be correct only by accident of one district.
