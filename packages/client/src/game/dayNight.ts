...
/** Время сна агентов: sleepStart..wakeStart через полночь. */
export function isSleepTime(hour: number): boolean {
  return hour >= NIGHT_SCHEDULE.sleepStart || hour < NIGHT_SCHEDULE.wakeStart;
}

/**
 * Тонировка района по времени суток: линейный лерп цвета и альфы между
 * ключевыми точками DAY_TINT_KEYS, по кругу через полночь.
 */
/**
 * Интенсивность ночного света 0..1: 0 днём, 1 ночью (NIGHT_LIGHT.start..end),
 * линейный fade за fadeHours на включении и выключении.
 */
export function nightIntensity(hour: number): number {
  const { start, end, fadeHours } = NIGHT_LIGHT;
  const h = hour >= start ? hour : hour + 24; // ночь без разрыва: start..end+24
  const endH = end + 24;
...
export function tintAt(hour: number): { color: number; alpha: number } {
  ...
  // ищем пару соседних ключей [prev, next], в интервале которых лежит hour
  ...
  for (let i = 0; i < keys.length; i++) {
    ...
    if (i === keys.length - 1) {
      // после последнего ключа — интервал до первого ключа следующих суток
      prev = keys[i];
      next = keys[0];
    }
  }
