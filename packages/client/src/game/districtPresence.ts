 ...
 *
 * Extracted verbatim from DistrictScene.syncAgents (the `present` filter and
 * the two loops that follow it). Behaviour is identical, deliberately: the
 * baseline captured from this module describes the scene as it was before
 * multi-district work began.
 *
...
export interface SyncInputs<A extends LocatedAgent> {
  ...
  /** Is there a door on this map keyed by that scene key? */
  hasDoorForScene: (sceneKey: string) => boolean;
...
/**
 * The district draws only those the server says are outside or at the farm.
 * The player entering/leaving has no effect on agent locations (TZ-16).
 *
 ...
 * agent at the farm is drawn by this scene — which is why it belongs here
 * alongside 'district' rather than being treated as somewhere else. The
 * fixture server emits it: agentLife.ts:37 puts 'farm' in the human daytime
 * pool, :38 in the animal pool, and :100 sends EVERY animal to the pen at
 * night. Drop it here and the animals vanish nightly and updateNightBehavior
 * loses its subjects. `packages/shared` AGENT_LOCATIONS and presence.ts's
 * liveVenueLookup both carry it deliberately.
 */
export function isDrawnHere(location: string): boolean {
  return location === 'district' || location === 'farm';
}
...
export function planSync<A extends LocatedAgent>(inputs: SyncInputs<A>): SyncPlan<A> {
  const { fullList, drawnIds, lastLoc, hasDoorForScene, isAsleep, isLeaving } = inputs;

  const present = fullList.filter(a => isDrawnHere(a.location));
  ...
  for (const id of drawnIds) {
    ...
    const hasDoor = hasDoorForScene(sceneKeyFor(newLoc));
  ...
  for (const a of present) {
    ...
    const atDoorOf = from && from !== 'district' && hasDoorForScene(sceneKeyFor(from))
      ? from
      : undefined;
