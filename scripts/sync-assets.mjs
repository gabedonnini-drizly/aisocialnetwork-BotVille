#!/usr/bin/env node
/**
 * Шаг 1 ТЗ-01: пайплайн ассетов.
 * Копирует ТОЛЬКО используемые файлы из assets-src/ (не в git, лицензия LimeZu
 * запрещает редистрибуцию) в packages/client/public/assets/ (тоже в .gitignore).
 *
 * Список файлов ЯВНЫЙ — сборка воспроизводится: купи паки, распакуй в
 * assets-src/ (структура — в README, раздел «About the art») и запусти
...
/** [источник в assets-src, назначение в public/assets] */
const FILES = [
  // ---- Тайлсеты района (Modern Exteriors) ----
  ...
  // ---- Room Builder'ы и интерьерные темы ----
...
// ---- Здания и пропсы района (готовые одиночные PNG) ----
const PROPS = [
  ...
  // library_building.png генерируется build-district.mjs (штампуется вывеска BOOKS)
  ...
  // проезжающие машины (амбиент дня/ночи)
...
// ---- Одиночные офисные объекты для OfficeScene (номера из пака) ----
...
// ---- Люди-агенты: 12 premade-персонажей (см. AVATAR_VARIANTS в assetManifest.ts) ----
...
console.log(`sync-assets: скопировано ${copied}/${FILES.length}`);
if (missing.length) {
  console.error('ОТСУТСТВУЮТ в assets-src/ (см. README, раздел «About the art»):');
