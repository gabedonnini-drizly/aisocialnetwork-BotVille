...
| **6 — Art & deployment** | 3, 3b, 20, 35, 38b, 39 | 1–5 | Real pixels, real deploys | Golden gate green; self-hosted Docker deploy works (D-20, 2026-07-30); LimeZu credit shipped; hero re-rendered |
...
| 35 | Deployment: retire Vercel/Railway, self-host via Docker (D-20) | 6 | no |
...
npm run bake:agents -- --roster roster/roster.json  # roster created in Task 35 Step 4
docker compose build && docker compose up -d && curl -sSf http://localhost:8080/ >/dev/null && echo "docker OK"   # self-hosted deploy (D-20, 2026-07-30) — replaces the retired Vercel/Railway checks
```
...
| **I-12** no art in a public build | `test/deploy-config.test.mjs` — a plain `docker compose build` (no `BOTVILLE_PACK`) cannot contain licensed art (D-20, 2026-07-30); `.dockerignore` and the `PACK=fixture` default exclude `assets-src` from every image |
...
| **G-H** BotVille is containerised | `test/deploy-config.test.mjs`; `docker compose up` in Task 35 Step 4 — the deployment itself, not a parity check alongside Vercel/Railway (both retired, D-20, 2026-07-30) |
