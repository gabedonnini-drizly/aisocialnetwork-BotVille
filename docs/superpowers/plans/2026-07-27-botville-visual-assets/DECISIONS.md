...
| D-9 | **U-1 is answered: the pack ships separable character layers.** Body / eyes / hair / outfit / accessories as independent PNGs — high confidence, documentary (see `ART-PACK.md` for the evidence and its limits). **`capabilities.characterLayers` should be `true` from the start**, not defaulted `false` and flipped 34 tasks later. |
