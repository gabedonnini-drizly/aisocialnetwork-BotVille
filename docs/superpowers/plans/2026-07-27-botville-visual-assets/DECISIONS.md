...
| D-20 | **Deployment is self-hosted, like BotTown** (2026-07-30). Local dev servers for development; hosting on the owner's own server the same way the BotTown api and frontend are hosted — both are node apps, Docker-packaged for convenience. **Vercel and Railway are not used**: `vercel.json`, `railway.toml`, `scripts/deploy-server.mjs` and the `deploy:*` scripts are retired legacy config. The licensed-art rule is unchanged and applies to self-host images: no raw sheets or `assets-src/` in any image pushed anywhere; real-art bakes happen on the owner's server (or are volume-mounted), never committed. |
| D-19 | **Use ALL pack variants — no manual curation** (2026-07-30, supersedes D-16's owner-pick-12/8). The hair/outfit axes derive from the full pack: 29 hairstyle styles × their built-in color-variant files (200 sheets) and 33 outfit styles × variants (132 sheets), excluded only by AUTOMATED checks (sleep/sit-row coverage per D-17, palette separation where recoloring still applies). "If a cap is ever technically required, the first set meeting requirements wins — never a taste pass." Variant lists are committed generated data derived from the pack; changing the pack re-rolls derived appearances (accepted). |
