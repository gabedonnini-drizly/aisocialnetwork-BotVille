...
In `config.ts:28`, replace `/** District map (must match scripts/build-district.mjs). */` with:
...
In `InteriorScene.ts:34` replace `(scripts/build-interiors.mjs)` with `(scripts/world-bake.mjs)`. In `PreloaderScene.ts:39` replace `(generated by scripts/build-district.mjs)` with `(generated by scripts/world-bake.mjs)`.

In `scripts/sync-assets.mjs:81`, the comment `library_building.png is generated by build-district.mjs (the BOOKS sign is stamped on)` → name `world-bake.mjs` instead. (Load-bearing: Task 19a retires that very list, and the `bookSign` generator moves to the adapter.)
