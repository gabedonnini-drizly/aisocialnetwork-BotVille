...
`docs/facts.yaml`.
**⚠ AMENDED (review 2026-07-31, A-12):** facts.yaml M-051 currently
states "no re-baselining round has been captured yet" and nothing cites
`run_20260731_084950` — this task must ALSO revise M-051's stale rider
(point it at M-052) in the same commit, or the corpus stays
contradictory. And cite **M-051**, not M-037 — M-037 is RETRACTED
(`facts.yaml:555-570`) even though its numbers are coincidentally true
again.
...
**Files:**
- **⚠ AMENDED (review 2026-07-31, A-2 — the grep was run; the answer is
  pinned):** first-firing-wins is the `for … return` loop INSIDE
  `_delegation_candidate` itself (`candidate_builder.py:703-711`);
  `heartbeat.py:558-563` only builds and orders the trigger list from
  subagent YAML `menu_triggers`. The lottery therefore replaces the
  selection rule *inside* `_delegation_candidate` (call
  `select_delegation_trigger` there); the function's public signature
  can stay, but "the consumer keeps its signature, the producer moves"
  was the wrong model — there is no separate producer site.
...
- Create: `heartbeat/infra/adapters/city_state_client.py`
  **(⚠ AMENDED, review 2026-07-31, A-8: `adapters/http/` does not exist;
  the house convention is flat `*_client.py` modules beside
  `md_gen_client.py` / `commit_http_client.py` — do not introduce a new
  directory convention)**
- Modify: `heartbeat/app/bootstrap.py` — plain constructor-kwarg wiring
  (amended: the injection site is `bootstrap.py:479-490`
  `HeartbeatOrchestrator(...)`; there is no DI container)
- Test: `tests/heartbeat/unit/test_city_state_port.py` (create)
- **Auth note (RULED D-56):** the endpoint is public now, but the
  adapter is built config-auth-ready — it sends the internal-token
  header IFF its env token is set (unset today). Test both arms so the
  later flip is a config change, not code.
...
- Modify: `heartbeat/infra/adapters/crew/exposure_log.py` —
  **(⚠ AMENDED, review 2026-07-31, A-1: the file is INFRA, not
  `core/orchestration/` — no such core file exists; extractors at
  :354-389, registered :404-406)** — re-verify the BotVille extractors
  against Plan 01 Task 5's reworked `get-city-goals` payload
...
- `_city_candidate` implements the spec §VI.2 priority ladder verbatim:
  ...
  rendered text; the builder never invents copy).
  **⚠ AMENDED (review 2026-07-31, D-e/D-f — the ref mechanism,
  verified in both repos):**
  - The original "reusing registered non-ack-able kinds" was FALSE —
    `ExposureRef.kind` is a free string; nothing registers `place`, and
    `city_goal`/`city_proposal` exist nowhere. They are NEW kind
    strings.
  ...
  - City-kind refs (`city_goal`/`city_proposal`/`place`) ride the
    ...
    now stated). Test: a wake with a city candidate commits acks
    containing NO city-kind refs, and the round-(b) probe shows one
    commit ACCEPTED with the new kinds present in `shown_refs`.
...
- Modify: `heartbeat/infra/adapters/crew/unified_runner.py`
  (`EXCLUDED_TOOLS` — currently 22 = 7 L3 + 15 L2 [M-051 — **amended
  A-11: M-037 is RETRACTED**, cite M-051]; the new
...
- Test: extend `tests/heartbeat/unit/test_delegation_lottery.py`
...
- [ ] Suite → green. Commit:
  `feat(delegation): city_propose vacuum trigger + reflector city reads (D-49)`
- [ ] **ROUND (b)** — Tasks 2+3+4+5 deploy together as ONE agent-facing
  ...
  `shown_refs` (amendment D-e).** Analyzer sections: city candidate
  offered (from manifests — the ack ledger cannot see city kinds,
  amendment D-e)/truncated/chosen; **DB-side receipt counts beside
  episode counts**; vote receipts; trigger fired/won/chosen; decision
  mix vs M-053; ≥10 raw-trace reads from this round's own log window.
  Register **M-055**. Re-derive D-40 coefficients from measured
  participation; record the revision.
  **RULED D-54 (review session):** the behavioral-eval hard floors
  (`scripts/admin_tools/llm_judge.py`) gain a city criterion — ≥1 city
  action per engaged agent — effective from this round, landing
  TOGETHER with the awareness surfaces this round ships (28-schema
  catalog, city candidate, specialist block): floors and awareness
  together, never floors alone. If round (b)'s measured selection rate
  shows the floor is unreachable, that is a tuning signal on the
  awareness surfaces, reported in the analyzer — not a reason to
  silently drop the floor.

## Task 6: Ambient placement (D-48) → ROUND (c)

**✅ RESOLVED — D-53 (review session, DECISIONS.md):** the owner
ratified exactly this implementation — placement sourced from
`CityStatePort.placement` (one fetch, one presence truth, no II.1
rule-3 exception), compiled into the soul prompt's "Right Now" section,
lifecycle-harness-tested. Owner rationale in DECISIONS.md D-53
(consistency, no race conditions, simple over complex, leverage the
existing prompt lifecycle). Execute as written below.
...
- [ ] **ROUND (c)**: **probe first: the placement line present in one
  ...
  **Rider addendum (review, D-h):** the two soul-prompt consumers
  beyond the hashes — `observability._parse_soul_sections` (breaks on
  heading REMOVAL) and `microtest3_eot_replay.SOUL_SECTION_ORDER`
  (breaks on heading RENAME) — are unaffected because the line lands
  INSIDE the existing "Right Now" section via `_append_right_now`
  (`prompt_compiler.py:241-296`); assert no heading added/renamed, and
...
- Modify: the grounding gate.
  **⚠ AMENDED (review 2026-07-31, D-d — the grep was run; the pointer
  was wrong):** `strip_fabricated_ids` (`normalization.py:112`) is a
  pure regex strip with no manifest access and cannot ground anything.
  The real gate family is `_ground_continuation`
  (`end_of_turn.py:529-555`), which reads the session **witness sink**
  — and the BotVille extractors record venue **labels**, not ids
  (`exposure_log.py:354-362`), so the shown-venue-ID surface D-47
  grounds against DOES NOT EXIST YET. This task must (a) extend the
  BotVille extractors to record shown venue IDs (still shown-only,
  still zero ack refs), and (b) validate `venue_id` against that
  shown-ID set ∪ {home, workplace}; failure strips the anchor, keeps
  the promise.
- Modify: `heartbeat/core/orchestration/candidate_builder.py`
...
- [ ] **C8 FIRST (mandatory before edits):** trace promises→A-1
  ...
  **⚠ AMENDED (review 2026-07-31 — blast_radius was run; three
  couplings the original plan missed):**
  - `eot.promises` feeds **three** consumers, not one: `_own_intention`
    PLUS `_due_commitment` PLUS `heartbeat._apply_anchor_gate` (prior
    promises fold into the continuation-elision anchor set — a
    reshaped promise list changes which carried-forward sentences
    survive). Trace and test all three (D-a).
  - **Pipeline shape contract (D-c):** the model emits promises as
    BARE STRINGS (`end_of_turn.py:227`); `assign_promise_ids` builds
    the dicts; `_own_intention` requires `id`+`text`
    (`candidate_builder.py:449-450`) or silently falls to the
  ...
  - **Shared-instructions hazard (D-b):** the extraction-instructions
    string is shared by the prod legacy path and the dev commit path
    with no flag and no off-arm — landing this edit on the live
    checkout ships it to prod's next round. Do the work in a worktree;
    merge only inside round (d)'s deploy window, stated in the round
    bullet.
- [ ] Tests first:
...
- [ ] **ROUND (d)**: **probe first: one anchored promise survives
  ...
  Task-7 worktree merges only inside this deploy window (amendment
  D-b).** Extraction-prompt bytes moved → re-baseline; **count the
...
- Modify: `candidate_builder.py` — the §VI.2 nudge rung consumes
  `city_state.pending_nudges` (already rung 1 from Task 3). **⚠ AMENDED
  (review 2026-07-31, D-f):** decline semantics come from the ack
  ledger, not "derived from the exposure manifest" agents-side: the
  rung-1 ref is `kind='nudge'`, so shown-but-unchosen produces a
  `presented` ack and the affordances payload's any-ack filter (Plan 01
  Task 7) stops the re-offer. Nothing new to build here beyond Task 3's
  ref kind — test the presented case end-to-end against a fixture
  payload.
- Modify: `prompt_compiler.py` — praise renders once as the D-50
  ...
  `verb='praise'`.
  **⚠ AMENDED (review 2026-07-31, F-4 — the original pin was FALSE and
  is replaced):** there is no "`GET /api/nudges` consumption flow" —
  reads are non-destructive by design (`mdGenController.js:467-468`:
  "Nudges are only consumed by the explicit POST /nudges/ack"); the
  only consumers are the ack endpoint and the commit-path
  `exposureAckService.syncNudgeConsumed` (`:143-146`) on
  engaged/deferred acks. **Without a consumption write, praise renders
  every wake, violating D-50's exposed-once.** Mechanism (**RULED
  D-55** — nudges are a queue, delivered next wake, consumed on
  delivery): when the praise line renders, append
  its ref (`kind='nudge'`, the nudge id) to the wake's
  acknowledgements as `engaged` — the render IS the exposure;
  `syncNudgeConsumed` fires inside the commit; zero new machinery.
  Test: praise renders on wake N, is consumed by N's commit, absent
  from wake N+1's affordances payload.
...
- Modify (hygiene): `configs/defaults.yaml` stale comment — **amended
  (A-9): lines 31-32 only** (the dangling "L2: BotVille" section
  reference; lines 29-30 are CURRENT auth documentation, do not touch);
